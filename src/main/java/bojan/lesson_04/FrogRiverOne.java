...
public class FrogRiverOne {
    ...
    public int solution(int X, int[] A) {
        ...
        // create a new array that holds each leaf from 0..X
        boolean[] counterArray = new boolean[X];

        // counter of total leafs found
        int finalCounter = 0;
        ...
        for (i = 0; i < A.length; i++) {
            ...
            // if the leaf falls on a blank space
            if (!counterArray[A[i] - 1]) {

                // mark that space as having a leaf
                counterArray[A[i] - 1] = true;

                // increase the number of total leafes
                finalCounter++;

                // if all places from 0..X have a leaf,
                // that means we have total of X leafs -> success
                // no need to iterate all elements
                if (finalCounter == X) {
                    break;
                }
        ...
        // the element i marks the position in the original array
        // where we found all the leafs
        return finalCounter == X ? i : -1;
